/**
 * CLI entry point for pushing products to Shopify.
 *
 * Usage:
...
function showHelp(): void {
  console.log(`
Shopify Product Push CLI

Usage:
  npx tsx scripts/push-product.ts <styleID>    Push one product to Shopify
  npx tsx scripts/push-product.ts --setup      One-time store setup (creates Print Area definitions)
...
async function main(): Promise<void> {
  ...
  try {
    ...
    console.log(
      `Product created: ${result.productGid}, ${result.variantCount} variants, ${result.metaobjectCount} print areas`,
    );
  } catch (error) {
